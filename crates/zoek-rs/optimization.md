...
- **stage-3(다음) = `enclosing_symbol_id` Option<String>→u64**(−0.8GB, dedup 무관): compute_receiver_resolution(string fact-eq를 u64로) + serialize_reference_record(enclosing도 pre-parsed u64로, prereq 패턴) + serialize_ref_site + parse. enclosing_symbol_id_hash는 이미 precompute라 receiver_cache 키 불변.
