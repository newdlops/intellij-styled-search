...
- **★ W26 같은 세션 stage-5a 완료 (사용자 선택 A): reference writer(14M) → `RefWriteCol` 컬럼.** `append_lights`/`serialize_reference_binary_from_light`/`write_lights_from_channel`/`stream_lights_to_sidecars`가 ref_sites·file_table probe 없이 RefWriteCol만 읽음. **게이트: 75/75 + invariant 14,372,638 + bytes 3,788,145,402 (channel+fallback 둘 다).** **발견: prefilter column path(soa_b3)는 이미 완전 column-only** → 남은 column-path ref_sites 읽기 **단 2곳**(`build_phase_e_file_buckets` rel_path_hash + worker cold rel_path). 상세는 **full B6 진행 상황 stage-5 항목**.
...
- **stage-5 = `Vec<RefSite>` drop (−7.6GB 본체) — 사용자 선택 (A): 전 필드 컬럼화→OV1 유지**. 진행:
  - ✅ **5a 완료(W26)**: ① **reference writer(14M) → 컬럼**. 신규 `RefWriteCol{source_ref_id,enclosing_id,name_id,file_id,positions×4,edge_kind_id}`(~48B), `build_ref_write_cols`(par, file_table 완성 후). `append_lights_to_both_shards`/`serialize_reference_binary_from_light`/`write_lights_from_channel`/`stream_lights_to_sidecars`가 RefWriteCol만 읽음(ref_sites·file_table probe 제거; file_id 직접, edge_kind는 `edge_kind_str_from_id`→`serialize_reference_record`가 id 재도출이라 byte-identical). **게이트: 75/75 + invariant 14,372,638 + bytes 3,788,145,402(channel+fallback 둘 다).**
  - **★ 발견: prefilter(31M HOT) column path(soa_b3, default)는 이미 완전 column-only**(site_cols + self/cls/type_recv/star_rel hash). 문자열/ref_sites 읽기 0 → ② **prefilter 작업 불요**(struct path만 문자열, 그건 opt-out).
  - **남은 column-path ref_sites 읽기 단 2곳**: (5c) `build_phase_e_file_buckets`의 `ref_sites[idx].rel_path_hash`(→`site_cols[idx].rel_path_hash`) + worker cold(`compute_receiver_resolution`의 `site.rel_path`→**file_id 컬럼+`FileTable::get_path`**). 둘 다 column으로 변환 가능.
  - **(5b) ★ OV1 static writer `serialize_ref_site_binary`(38M disk shard, 전 필드)** = 최대 blocker. fresh ref_site는 edge∈{call,usage}·access∈{member,bare} **전부 standard**라 edge/access fallback 문자열 불요 → **language OTHER만 조사 필요**(미지 확장자 파일이 ref_site를 내는지; 0이면 단순, 있으면 side-table). 나머지 필드는 컬럼(name_id/receiver_name_id/file_id/positions/source_ref_id/enclosing_id + SiteCols의 language_id/edge/access/flags)로 복원.
  - **(5d) drop**: ⚠️ **SiteCols 빌드가 resolve 내부(~5840)** → drop하려면 **SiteCols + 전 컬럼을 resolve 전(rebuild_graph_native)으로 이동** + `stream_file_table`도 분기 전으로 이동(현재 channel은 resolve 전, fallback은 resolve 후 — 양쪽 빌드 동일하면 이동 안전) → resolve에 columns 전달 → resolve 진입 전 `drop(ref_sites)`. **struct opt-out 경로(soa OFF)는 ref_sites 읽으므로 drop과 양립 불가 → 제거(문서 W20 "B6와 묶음")하거나 drop을 soa-ON 한정.**
- 각 stage 게이트: 75/75 + invariant 14,372,638 + **bytes 3,788,145,402**(dedup/serialize divergence 포착). **stage-2/4 실증**: bijective relabel·string-key 복원은 invariant·bytes 둘 다 보존(게이트가 확인). OV1 ref_site write가 resolve 중이라 serialize도 columns화해야 최종 drop 가능.
...
### 이번 세션 (W26) — B6 stage-4 (name/receiver → interner) + stage-5a (reference writer → RefWriteCol 컬럼)
...
**남은 5b/5c/5d (다음 세션, −6GB 실현)**:
- **(5c) 남은 column-path ref_sites 읽기 단 2곳**:
  1. `build_phase_e_file_buckets`(graph.rs:~5601) `ref_sites[idx].rel_path_hash` → `site_cols[idx].rel_path_hash`(시그니처에 site_cols 전달; rel_path_hash는 SiteCols에 있음).
  2. worker cold(graph.rs:~6413, `compute_receiver_resolution`의 `site.rel_path`) → **file_id 컬럼 + `FileTable::get_path`**(역방향 이미 존재). receiver는 이미 stage-4서 복원. resolve에 file_table + file_id 컬럼 전달 필요(또는 bucket당 1회 복원).
- **(5b) OV1 static writer `serialize_ref_site_binary`(graph.rs:~11147, 38M disk shard 전 필드)**: 전 필드 컬럼 복원. **edge/access는 fresh ref_site가 전부 standard**(extract_ref_sites=call/usage·member/bare)라 fallback 문자열 불요. **language OTHER만 미지수** → 먼저 captain2서 language fallback 발생 횟수 측정(0이면 단순, 있으면 미지-language site의 side-table). 필드 출처: name_id/receiver_name_id/file_id/positions/source_ref_id/enclosing_id(컬럼) + language_id/edge_kind_id/access_kind_id/flags(SiteCols).
- **(5d) drop**: ⚠️ **SiteCols 빌드가 resolve 내부(graph.rs:~5840)** + `stream_file_table`이 분기별(channel=resolve 전, fallback=resolve 후) → **SiteCols + 전 컬럼 + file_table을 resolve 전(rebuild_graph_native)으로 이동**, resolve에 전달, resolve 진입 전 `drop(ref_sites)`. **struct opt-out 경로(soa OFF, prefilter 6151 / worker 6468 / phase_c)는 ref_sites 읽으므로** drop과 양립 불가 → **제거**(W20 "B6와 묶음")하거나 drop을 soa-ON 한정. fallback/incremental/legacy 분기는 RefSite 유지.
- **인프라 재사용**: stage-4 `name_interner`/`site_name_ids`/`site_receiver_name_ids` + stage-5a `RefWriteCol`(file_id/positions/source_ref_id 이미 보유!) + `FileTable::get_path` + SiteCols(B4). 5b/5c는 RefWriteCol을 거의 그대로 쓸 수 있음(resolve로도 전달).
- **게이트(매 step)**: 75/75 + invariant 14,372,638 + **bytes 3,788,145,402** + schemaVersion 20. + 5d 후 **peak RSS matched-idle 측정**(captain 종료, W24 15.72GB 기준 −6GB 기대 ~10GB).
