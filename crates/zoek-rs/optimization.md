...
- ✅ **stage-2 완료(W24)**: `source_ref_id` String→u64. `stable_ref_id_u64`+`site_partial_hash_u64` 신규(string판 `edge_key_hash`/`site_partial_hash` 제거). site_cols build·phase_f가 u64 site_partial(**dedup relabel — bijective라 invariant 보존, 게이트 확인됨**). materialize=`format!` 복원(cold), serialize/parse_ref_site=u64 직접. **검증: 75/75 + invariant 14,372,638 + bytes 3,788,145,402 + peak RSS 17.98→15.72GB(OV1 ON서 16GB 캡 아래 복귀!).**
- ✅ **stage-3 완료(W25)**: `enclosing_symbol_id` Option<String>→`enclosing_id: u64`. **접근 (C)**(계획 (B: helper들 Option<u64>화)보다 안전): symbol id가 항상 `format!("sym:{:016x}")`라 신규 `enclosing_id_to_string`로 **byte-exact 복원** 가능 → `compute_receiver_resolution`·`type_fact_applies_by_enclosing`·`resolve_type_fact_targets_by_key`·`resolve_type_name_targets_by_key`의 `Option<&str>` **시그니처 유지**(enclosing이 출력에 retain 안 돼 lifetime만 `&'a`→`&` 완화, 컴파일러 검증). receiver_cache 키 `(recv_hash, enclosing_symbol_id_hash)`→`(recv_hash, enclosing_id)` **bijective relabel**(둘 다 enclosing 문자열의 deterministic injective 함수, stage-2처럼 bytes-gate 확인). `enclosing_id` **non-skip 직렬화**(spill 생존 → FIXUP 재계산 제거). serialize_reference_record=pre-parsed u64+inline(prereq 패턴, 14M path format! 0); serialize/parse_ref_site_binary=u64 직접; materialize·enclosing-shard 키(shard_index_for_key는 string이라 read-호환 위해 유지)·per-file cache-miss=cold 복원만. dead 함수(combined_member_candidates·type_fact_applies_to_site·resolve_type_fact/name_targets)=삭제 대신 최소 갱신. **검증: 75/75 + invariant 14,372,638 + bytes 3,788,145,402 + schemaVersion 20(완전 byte-identical). peak RSS 단일측정 16.35GB/footprint 19.67GB(±1GB noise·captain idle 미확인이라 W24 15.72와 직접비교 불가; 코드상 −24B inline/site + enclosing String heap 제거는 명백, 큰 메모리 win은 stage-5).**
- ✅ **stage-4 완료(W26)**: `name`/`receiver_name`→NameInterner u32 id. **reverse `names: Vec<Box<str>>` 추가**(forward `ids`는 collision-free string-key → 복원 byte-exact). 분기 전 빌드: symbol name + 전 receiver intern → `site_name_ids`/`site_receiver_name_ids`(par populate). **write path(14M)** `serialize_reference_binary_from_light`가 `interner.name(name_id)`로 name 복원(**fallback 없음**, MISS면 bytes 발산 → 가설 게이트). **가설 입증**: emitted name == 타겟 symbol name이라 symbol interner로 14M 전부 hit. **worker cold** `receiver_recon: Option<(&NameInterner,&[u32])>`로 receiver 복원(byte-exact); rel_path는 아직 site. materialize/push 미변경(channel write path 아님). **검증: 75/75 + invariant 14,372,638 + bytes 3,788,145,402 + schemaVersion 20.** ⚠️ 전이적 +~330MB(stage-5서 회수).
- ✅ **stage-5 완료(W26) = `Vec<RefSite>` drop, byte-identical — 단 ⚠️ 메모리 win 없음(아래 발견).** 사용자 선택 (A: 전 필드 컬럼화→OV1 유지). 사용자 결정 = **유지(keep)**(byte-identical·더 깨끗한 구조·평균 메모리↓).
  - **5a**: reference writer(14M) → `RefWriteCol`{source_ref_id,enclosing_id,name_id,file_id,positions×4,edge_kind_id,+5b: language_id,access_kind_id,flags}. `append_lights`/`serialize_reference_binary_from_light`/`write_lights_from_channel`/`stream_lights_to_sidecars`가 RefWriteCol만(ref_sites·file_table probe 제거).
  - **5c**: `build_phase_e_file_buckets`→site_cols.rel_path_hash; worker cold rel_path→`recon.file`(file_id+`FileTable::get_path`). receiver는 stage-4 interner. **prefilter column path(soa_b3)는 이미 column-only라 무작업.**
  - **5b**: OV1 disk writer 신규 `serialize_ref_site_binary_from_cols`+`write_ref_sites_by_file_shards_cols`(RefWriteCol+SiteCols flags+`site_receiver_name_ids`+interner). **probe로 unknown-language ref_site=0 실측** → language/edge/access fallback 문자열 전부 불필요(전부 standard, id 바이트로 복원). interner를 **전 site name까지 확장**(disk writer가 38M 전체 name 직렬화, 비-symbol 6.7%도 복원 필요; symbol id 불변이라 5a reference writer byte-identical 유지).
  - **5d**: `build_site_cols` 헬퍼로 SiteCols를 resolve **전**(channel branch)에 빌드 후 resolve에 `prebuilt_site_cols`로 전달; `recon`+`force_columns`로 struct opt-out 경로 차단(`drop_ref_sites = overlap_static && soa_on`); 컬럼 빌드 후 `ref_sites = Vec::new()`로 7.6GB free. resolve 내부 `ref_sites.len()`→`site_cols.len()`(아니면 0). **버그2개 게이트가 포착**: (i) 길이 totals가 빈 ref_sites로 0→referenceCount=0, (ii) **phase_c 경계(scope/lang hash)·phase_f push(site_partial)가 column path서도 ref_sites[i]를 읽던 5c 누락분** → file_recon/language_str_from_id·`site_cols.site_partial`로 수정. fallback/legacy/incremental은 ref_sites 유지(drop_ref_sites=false).
  - **게이트: 75/75 + invariant 14,372,638 + bytes 3,788,145,402(channel **+ fallback** 둘 다 byte-identical) + schemaVersion 20.** 5a~5d 각 단계 게이트 통과.
- **★★ 핵심 발견 — B6 전제가 틀렸다 (메모리, paired로 확정)**: **drop ON/OFF paired**(신규 측정 게이트 `ZOEK_B6_KEEP_REFSITES`로 drop만 토글, 컬럼·column-path 동일; peak RSS는 프로세스 자체값이라 captain load 무관·신뢰):
  - **ON(drop): 15.19 / 14.94GB (avg ~15.07)** vs **OFF(keep ref_sites): 15.91 / 15.16GB (avg ~15.54)** — 전부 byte-identical(3,788,145,402).
  - **→ ref_sites(7.6GB) 해제가 peak를 ~0.5GB만 줄인다(7.6 아님).** **peak는 build-window**(parse가 ref_sites+symbols+facts ~9.1GB 빌드 후 그 위에 컬럼 ~4GB 적층 → ON/OFF 동일)이고, drop은 그 *이후*라 resolve tail(피크 아래)만 깎음. parse-only peak=9.10GB로 parse 자체가 base.
  - **결론: −6GB는 ref_sites 은퇴로 불가능 — 확정.** W24 stages 1-2의 −2.26GB는 ref_sites *축소*가 build-window를 직접 줄였기 때문(작동), 하지만 *완전 제거*는 컬럼이 그 자리를 메우고 parse base가 남아 무이동. **진짜 메모리 레버 = ① parse base(ref_sites+symbols+facts ~9GB at parse) 축소, ② 컬럼을 ref_sites와 공존 안 하게(불가) 또는 resolution 맵 축소.** B6(resolve-time ref_sites drop)는 peak 레버가 아님.
  - **⚠️ wall 미측정**: 5a가 14M write path의 per-record `file_table.get_id` 제거(소폭 이득), 5b가 site-name 전체 intern 추가(parse 후 sequential ~+0.5-1s 추정 비용) — net wall은 **captain-idle matched-paired 필요**(현재 미확보)라 미정량. 별도 idle 세션 권장.
- 각 stage 게이트: 75/75 + invariant 14,372,638 + **bytes 3,788,145,402**. **stage-2/4/5 실증**: bijective relabel·string-key/file_id 복원은 invariant·bytes 보존; **drop은 길이·잔존 reader 누락을 bytes 게이트가 포착**(phase_c 경계 + phase_f push).
...
### 이번 세션 (W26) — B6 stage-4 (name/receiver → interner) + stage-5 완료 (Vec<RefSite> drop, byte-identical, 단 peak 메모리 win 없음)

**먼저 한 일**: optimization.md(W25까지) 읽고 build green + 75/75 재확인. 진입점 지정 **B6 stage-4** 착수. 핵심 난점 = **MISS 복원 문제**: interner는 symbol 이름만 알아 site의 `name`이 비-symbol이면 MISS(u32::MAX) → 복원 불가. 그래서 stage-5(`Vec<RefSite>` drop) 토대로 interner+컬럼을 깔되, **복원이 byte-exact임을 게이트로 증명**하는 게 stage-4 목표.

**핵심 설계 결정 — "emitted name은 항상 symbol name" 가설 + B6a probe 데이터**:
- **B6a probe(`ZOEK_SOA_B1`) 실측**: `interner_build=269ms unique_names=614,994 populate_by_str=72ms sites=38,222,730 interned_hit=35,654,140 (93.3%)`. 38M 중 6.7%(2.57M) site는 비-symbol 이름.
- **가설(코드 추적으로 입증)**: emitted reference(14.37M)의 `name`은 **항상 타겟 symbol의 name**이다 — bare는 `bare_symbols_by_name.get(name_hash)`로, member는 `members_by_container_and_name(_h)`로 site name을 **symbol-keyed 맵에 매치**해야 candidate가 나오므로 site.name == target.name. 따라서 emitted name은 전부 symbol interner에 있고, 비-symbol site name(MISS)은 **resolve 자체가 안 돼 write path에 도달 안 함**. → **symbol-name interner만으로 14M write 전부 복원 가능**(검증은 bytes 게이트).
- receiver는 다름: local/`self`/`cls` 등 비-symbol일 수 있어 **별도 intern 필요**. 단 receiver string은 **column worker의 per-file cache-miss(cold)서만** 복원(hot path는 receiver_name_hash 사용). `populate_by_str=72ms`라 collision-free string-get 복원이 충분히 쌈.

**구현 (요약, 모두 byte-identical)**:
- **NameInterner reverse table**: `names: Vec<Box<str>>`(id→&str) + `name(id)->Option<&str>` 추가. forward `ids: AHashMap<Box<str>,u32>`는 **collision-free string-key 유지**(복원 정확성), reverse는 2번째 Box<str>(중복 ~27MB — 7.6GB 대비 무시).
- **빌드(분기 전, `rebuild_graph_native`)**: symbol name intern + 전 receiver intern → `site_name_ids`/`site_receiver_name_ids`(`rayon::join` par populate, `interner.get` = collision-free). channel writer thread가 resolve와 **동시** 소비하므로 scope 진입 전 빌드(resolve 내부 아님). channel+fallback 양쪽 분기가 공유.
- **write path(14M, name 복원)**: `serialize_reference_binary_from_light`가 `name: &str`(caller 복원)을 받아 `site.name`(×2, name+raw_text) 대신 사용. `append_lights_to_both_shards`가 `name_interner.name(site_name_ids[light.site_idx])`로 복원(**fallback 없음** — MISS면 ""로 bytes 발산 → 게이트가 가설 검증). plumbing: `write_lights_from_channel`→`append_lights_to_both_shards`→`serialize_reference_binary_from_light` + fallback `stream_lights_to_sidecars`(2 callsite).
- **worker cold(receiver 복원)**: `resolve_ref_sites_a_to_e`에 `receiver_recon: Option<(&NameInterner,&[u32])>` 추가(`resolve_ref_sites_for_rebuild` pass-through). column path cache-miss(graph.rs:~6390)서 `Some`이면 `interner.name(recv_ids[site_idx])`, `None`(incremental/legacy/test)이면 기존 `site.receiver_name`. string-key intern이라 byte-exact. **`rel_path`는 아직 site 읽음**(stage-5 file_id 작업).
- **materialize_light_ref/push_resolved_reference 미변경**: channel write path가 아니라 **legacy/test/incremental 전용**(GraphReference materialize). captain2 게이트는 append_lights(write path)를 거치고, stage-5의 channel-path Vec drop서도 이들은 안 불림 → ref_sites 그대로 읽게 유지.

**검증/측정**:
- build green(기존 dead_code warning 외 0), **75/75**.
- captain2 **referenceCount=14,372,638 + symbolCount=4,141,426 + bytes=3,788,145,402 + schemaVersion=20** = **완전 byte-identical**. write-path name 복원 게이트(가설 입증: emitted name == symbol name, MISS 0) + worker receiver 복원 게이트 **둘 다 통과**(2번 별도 rebuild로 확인).
- **메모리**: stage-4는 **전이적으로 +~330MB**(interner ~30MB + name_ids 152MB + receiver_ids 152MB)를 `Vec<RefSite>`(~7.6GB) **위에 얹음** → peak ↑(16GB cap 일시 초과 가능, captain2 머신 RAM 넉넉). **진짜 win은 stage-5(Vec<RefSite> drop −7.6GB)**. stage-4 단독은 메모리 net 음수 — 의도된 토대.

**솔직한 진단 / 교훈**:
- **가설을 게이트로 증명**: "emitted name은 symbol name"이라는 코드 추적 결론을 **fallback 없는 복원 + bytes 게이트**로 검증 — bytes 동일 = 14M 전부 hit(MISS 0). 추론을 실측으로 잠갔다(문서 가치).
- **collision-free 복원**: hot path는 name_hash(u64) 쓰지만 **복원은 string-key interner**(get/name)라 hash 충돌 위험 없이 byte-exact. stage-2/3의 hash relabel과 달리 복원은 정확성이 생명이라 string-key 채택.
- stage-4는 **wall 목표 무관**(오히려 +interner 빌드 ~0.5-1s + 메모리 +330MB). 토대일 뿐, −6GB 본체는 stage-5.
- ⚠️ B1 probe(`ZOEK_SOA_B1`, resolve 내부)는 이제 실 빌드로 **대체됨**(probe는 측정용으로 잔존, default OFF — 무해하나 다음 세션서 제거 고려).

**stage-5 진행 (사용자 선택 = A: 전 필드 컬럼화 → OV1 유지)**:

- ✅ **5a 완료 (이번 세션)**: **reference writer(14M) → `RefWriteCol` 컬럼**. 신규 struct `RefWriteCol{source_ref_id u64, enclosing_id u64, name_id u32, file_id u32, positions×4 u32, edge_kind_id u8}`(~48B) + `build_ref_write_cols`(par, **file_table 완성 후** — file_id 직접). `append_lights_to_both_shards`/`serialize_reference_binary_from_light`/`write_lights_from_channel`/`stream_lights_to_sidecars` 시그니처를 `&[RefWriteCol]`+interner로 교체 → **ref_sites 읽기·per-record `file_table.get_id` probe 둘 다 제거**. edge_kind는 `edge_kind_str_from_id(col.edge_kind_id)`(reference는 항상 call/usage standard)로 복원, `serialize_reference_record`가 `compute_edge_kind_id`로 id 재도출 → byte-identical. **게이트: 75/75 + invariant 14,372,638 + bytes 3,788,145,402 (channel+fallback 둘 다).**
- ✅ **prefilter (구 #2) 작업 불요로 판명**: column path(`soa_b3`, default, graph.rs:~6070)는 **이미 완전 column-only** — `site_cols[i]` + 사전계산 `self_hash`/`cls_hash`/`type_recv_hashes`/`star_rel_hashes`만 읽고 ref_sites·문자열 0. (string 읽는 건 `ZOEK_SOA_OFF` struct path뿐 → 5d서 제거/우회.)

**5b/5c/5d 완료 (이번 세션) — byte-identical, 단 메모리 win 없음**:
- **(5c)** `build_phase_e_file_buckets`→`site_cols[idx].rel_path_hash`; worker cold rel_path→`recon.file`(file_id+`FileTable::get_path`). `SiteReconCols{interner, receiver_name_ids, file: Option<(&FileTable,&[u32])>}`로 receiver+rel_path 통합. prefilter는 이미 column-only(무작업).
- **(5b)** 신규 `serialize_ref_site_binary_from_cols`(RefWriteCol+`site_receiver_name_ids`+interner) + `write_ref_sites_by_file_shards_cols`(shard는 `file_table.get_path(file_id)`로 재구성). **probe로 unknown-language=0 실측** → language/edge/access 전부 standard라 id 바이트로 복원(fallback 문자열 0). **interner를 전 site name으로 확장**(disk writer가 38M 전체 name 직렬화 — 비-symbol 6.7%도 필요; symbol id 불변이라 5a byte-identical 유지). RefWriteCol에 language_id/access_kind_id/flags 추가.
- **(5d)** `build_site_cols` 헬퍼 → channel branch에서 resolve 전 빌드 → resolve에 `prebuilt_site_cols`+`force_columns` 전달; 컬럼 빌드 후 `ref_sites=Vec::new()`(가드 `drop_ref_sites=overlap_static&&soa_on`). resolve의 `ref_sites.len()`→`site_cols.len()`. struct opt-out 경로는 force_columns로 차단(제거 안 함 — ZOEK_SOA_OFF는 fallback용 유지). **bytes 게이트가 2버그 포착**: 길이 totals 0화(referenceCount=0), phase_c 경계(scope/lang hash)·phase_f push(site_partial)의 잔존 column-path ref_sites[i] 읽기(5c 누락) → file_recon/language_str_from_id·`site_cols.site_partial`로 수정.
- **★★ 핵심 발견(메모리, paired 확정)**: drop ON/OFF paired(`ZOEK_B6_KEEP_REFSITES`, peak RSS=프로세스값): **ON 15.19/14.94(avg ~15.07) vs OFF 15.91/15.16(avg ~15.54), 전부 byte-identical**. **drop이 peak를 ~0.5GB만 줄임(7.6 아님)** — peak는 **build-window**(parse 9.1GB + 컬럼 4GB 적층, ON/OFF 동일), drop은 그 이후라 resolve tail만 깎음. parse-only=9.10GB. **B6로 −6GB 불가능 확정.** W24 stages1-2 −2.26GB는 ref_sites *축소*가 build-window를 직접 줄여 작동했지만 *완전 제거*는 컬럼이 자리 메움. 진짜 레버=parse base 또는 resolution 맵 축소.
- **사용자 결정 = 유지(keep)**: byte-identical(channel+fallback)·resolve 중 ref_sites 없는 더 깨끗한 구조·평균 메모리↓. wall 이득(5a의 per-record file_table.get_id 제거 등)은 **matched-idle paired 미측정**.
- **다음 진짜 레버 = resolution 맵 축소**(peak 실주범). B6는 메모리 목표 미달로 종료(추가 ref_sites 컬럼화 무의미). 측정주의: 단일 peak ±1GB 노이즈·captain idle 미확인.

---

### 이번 세션 (W25) — B6 stage-3: RefSite.enclosing_symbol_id Option<String> → enclosing_id u64

**먼저 한 일**: optimization.md(W24까지) 읽고 build green + 75/75 재확인. 진입점이 지정한 **B6 stage-3**(full-B6의 3/5단계) 착수 — RefSite의 enclosing 문자열(38M)을 u64 컬럼화해 메모리를 더 내리고 stage-5(Vec<RefSite> drop) 토대 마련.

**핵심 설계 결정 — 계획된 (B) 대신 (C) 채택**:
- doc가 매핑한 stage-3 plan은 **(B)**: `compute_receiver_resolution` + type-fact helper들의 site enclosing param을 `Option<&str>`→`Option<u64>`로 바꾸고(시그니처 cascade), fact 비교 시 fact 문자열을 parse. 하지만 enclosing은 `symbols_by_id.get(id)`(7348/7585, **string-keyed**) lookup에도 쓰여 (B)는 u64-keyed symbol map(추가 메모리) 또는 어차피 string 복원이 필요 → 이득 불명확 + 큰 cascade.
- **사실 검증으로 (C)로 전환**: `stable_symbol_id`(graph.rs:12841)가 symbol id를 **항상 `format!("sym:{:016x}", hash)`**로 생성하고 `parse_stable_symbol_id_to_u64`가 그 정확한 역함수(16 hex, lowercase) → **`format!("sym:{:016x}", id)` 복원이 원본 `symbol.id`와 byte-exact**. 따라서:
  - resolve helper들의 `Option<&str>` 시그니처 **그대로 유지**, enclosing **lifetime만 `&'a`→`&`로 완화**(`compute_receiver_resolution`/`resolve_type_fact_targets_by_key`/`resolve_type_name_targets_by_key`/`combined_member_candidates_by_key`). 근거: enclosing은 `symbols_by_id.get`/string-eq에 **transient**하게만 쓰이고 출력(`ReceiverResolution<'a>`/`Vec<&'a GraphSymbol>`)에 **retain 안 됨** → 짧은 lifetime의 임시 복원 string 허용. 컴파일러가 건전성 보장(retain하면 컴파일 실패).
  - hot loop은 u64만 읽고, **cold 경로**(per-file receiver_cache miss 2곳 + dead 함수 + materialize 2곳 + enclosing-shard 키)서만 신규 helper `enclosing_id_to_string(u64)->Option<String>`로 복원.
- **(C)가 (B)와 동일 메모리 win**(String 필드 제거) + **훨씬 적은 코드/위험**(intricate resolution cascade 안 건드림). invariant-critical한 cache-key relabel은 둘 다 동일.

**구현 (요약, 모두 byte-identical)**:
- **struct**: RefSite `enclosing_symbol_id: Option<String>` 드롭 + `enclosing_symbol_id_hash: u64`(skip) → **`enclosing_id: u64`(non-skip 직렬화)**. SiteCols `enclosing_symbol_id_hash`→`enclosing_id`.
- **constructor**: `extract_ref_sites`는 enclosing을 **per-line 1회 parse**(`line_enclosing_id` — was per-site String clone+re-hash → 둘 다 제거, parse alloc↓). `parse_ref_site_binary`는 marker(0/1/2)→u64 직접(string 복원 0).
- **FIXUP(load 후)**: enclosing hash 재계산 블록 제거(enclosing_id가 직렬화돼 spill 생존).
- **receiver_cache 키**(B4 col + struct 경로): `(recv_hash, enclosing_symbol_id_hash)`→`(recv_hash, enclosing_id)`. cache-miss서 `enclosing_id_to_string` 복원해 unchanged helper에 `Option<&str>` 전달.
- **write**: `serialize_reference_record` 시그니처 `Option<&str>`→`enclosing_id_u64: u64, enclosing_inline: Option<&str>`(stage-2 source_ref_id prereq 패턴). caller 2: GraphReference 경로=parse 1회, from_light(14M)=`site.enclosing_id` 직접(format! 0). `serialize_ref_site_binary`=u64 직접(marker 1/0). `append_lights_to_both_shards`(14M)=enclosing-shard 키가 `shard_index_for_key(&str)`라 **read 호환 위해 call/construct enclosing에서만 cold 복원**.
- **materialize**(`materialize_light_ref`/`push_resolved_reference`)=`enclosing_id_to_string` 복원(`GraphReference.enclosing_symbol_id: Option<Box<str>>`).
- **dead 함수**(crate-wide caller 0: `combined_member_candidates`/`_by_key`/`type_fact_applies_to_site`/`resolve_type_fact_targets`/`resolve_type_name_targets`) — 삭제 대신(이전 세션 "kept for caller" 의도 존중 + scope 최소화) 컴파일 위해 최소 복원/위임.

**검증/측정**:
- build green(기존 dead_code warning 외 0), **75/75**, captain2 **referenceCount=14,372,638 + symbolCount=4,141,426 + bytes=3,788,145,402 + schemaVersion=20** = **완전 byte-identical**.
- single rebuild wall=35.3s(resolve 22.8 / parse 9.5 / index 0.83), 단 단일 run이라 비교 불가(doc 측정원칙).
- **peak RSS 단일측정 16.35GB(max RSS) / 19.67GB(footprint)**: ⚠️ W24 15.72GB와 직접 비교 불가 — (a) doc 명시 **±1GB run-to-run noise**(OV1 버퍼 타이밍), (b) **captain idle 미확인**. **코드상 −24B inline/site + per-site enclosing String heap 제거는 명백**하나, peak는 OV1 write 버퍼가 지배 → **진짜 큰 메모리 win은 stage-5(Vec<RefSite> drop)**. 인증 peak는 captain 종료 matched-idle로 추후.

**솔직한 진단 / 교훈**:
- **포맷 보장이 안전한 단순화를 열었다**: symbol id가 canonical `sym:{:016x}`임을 `stable_symbol_id`서 확인하니, doc가 계획한 helper-cascade(B) 없이 cold 복원(C)만으로 byte-identical 달성. **계획을 사실로 검증 후 더 안전한 쪽으로 조정**(doc 가치: profile/사실 우선).
- **lifetime 완화는 type cascade가 아니다**: `&'a`→`&`는 컴파일러가 retain-여부를 검증하는 lossless 변경 — (B)의 type+body 변경보다 위험 낮음.
- bytes-gate가 또 핵심: enclosing의 cache-key relabel(hash→id)·serialize u64화·복원 정확성을 한 번에 확인(invariant만으론 부족, stage-2 교훈 재확인).
- stage-3는 **wall 목표 무관, 메모리 토대**. 남은 stage-4(name/receiver interner)+stage-5(Vec<RefSite> drop)가 −6GB의 본체.
