...
**다음 세션 = full B6 (RefSite AoS retire) — fresh 세션 권장(W24서 매핑 완료, dedup/write-path cascade라 한 호흡에)**:
- **순서**: 먼저 **공통 prerequisite** = `serialize_reference_record`(graph.rs:10392, 14M-record write path)의 `source_ref_id:&str`/`enclosing_symbol_id:Option<&str>`를 **pre-parsed u64**로 받게 변경(callers 2: LightRef path 10489=u64 직접, GraphReference path 10372=parse 후 전달; sentinel u64::MAX+inline 유지). 이래야 필드→u64 전환이 write path서 14M 문자열 재구성 없이 byte-identical.
- **stage-2 = `source_ref_id` String→u64** (−1.5GB): RefSite 필드 + `stable_ref_id_u64`(새 fn, parse서 string 안 만듦) + `site_partial_hash_u64`(새 fn) — site_cols build(5767)·phase_f(8295)가 u64 기반 site_partial 사용(**dedup relabel: bijective라 invariant-safe, 게이트가 확인**). materialize(8596/8631)=`format!("ref:{:016x}")` 복원(cold). serialize_ref_site(10957)=u64 직접. parse_ref_site_binary=u64 직접 저장.
- **stage-3 = `enclosing_symbol_id` Option<String>→u64**(−0.8GB, dedup 무관): compute_receiver_resolution + serialize 경로.
- **stage-4 = `name`/`receiver_name`→u32 name_id**(NameInterner + **reverse `Vec<Box<str>>` 추가**): resolve readers + ref_site write가 interner로 복원.
- **stage-5 = `Vec<RefSite>` drop**: 위 전부 columns화되면 resolve 진입 전 drop. **−6GB → peak ~11-12GB(OV1 포함 캡 내).**
- 각 stage 게이트: 75/75 + invariant 14,372,638 + **bytes 3,788,145,402**(dedup/serialize divergence를 잡는 핵심). OV1이 ref_site write를 resolve 중 하므로 serialize도 columns화해야 drop 가능.
- **W24 교훈**: stage-2가 "빠른 slim"이 아니라 dedup-relabel+write-path 시그니처가 맞물린 cascade라, 긴 세션 끝이 아니라 fresh 세션서 실행(미묘한 버그 회피).
