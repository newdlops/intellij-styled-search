...
- **★★ 핵심 발견 — B6 전제가 부분적으로 틀렸다 (메모리)**: drop 후 **peak RSS 16.05GB(max) / 17.73GB(footprint)**, W24 15.72와 **사실상 동일(노이즈 내, win 없음)**. **parse-only peak=9.10GB** → peak는 resolve 단계지만 **resolution 맵들(`symbols_by_file_and_name`/`members_by_container_and_name_h`/dedup/light 버퍼 ~10GB)이 지배, ref_sites가 아님.** ref_sites(7.6GB) 전부 drop해도 추가 컬럼(~4GB: write_cols/site_cols/id들/interner)이 상쇄 + 맵들 불변 → net ~0. **W24 stages 1-2의 −2.26GB는 ref_sites가 *여러* 기여자 중 하나였단 뜻**이고, 완전 제거+컬럼추가는 peak 무이동. **→ −6GB는 ref_sites 은퇴로 불가능. 진짜 레버 = resolution 맵 축소(별도 최적화).** (단일 측정·captain idle 미확인이라 ±1GB 노이즈; 그래도 −6GB 목표는 명백히 미달.)
...
- **★★ 핵심 발견(메모리)**: drop 후 **peak 16.05GB(max)/17.73GB(footprint) ≈ W24 15.72(win 없음)**. **parse-only=9.10GB** → peak는 resolve지만 **resolution 맵(symbols_by_file_and_name/members_h/dedup/light 버퍼 ~10GB)이 지배, ref_sites 아님.** ref_sites(7.6) drop을 추가 컬럼(~4GB: write_cols/site_cols/id/interner)이 상쇄+맵 불변 → net 0. **B6로 −6GB 불가능 확정.** (W24 stages1-2 −2.26GB는 ref_sites가 여러 기여자 중 하나였단 의미.)
