...
structured/diagnostic output에서는 `content`에 compact text를 함께 제공하고, `structuredContent`에는 같은 데이터를 JSON object로 넣는다. token-first compact text tool은 기본적으로 `structuredContent`를 생략할 수 있으며, 필요한 경우 `include_diagnostics=true` 또는 `structured=true`로 명시한다.
...
6. MCP는 index refresh/rebuild를 시작하지 않는다. 사용자가 extension command나 CLI로 index를 준비하고 MCP는 readiness만 보고한다.
```
...
| `codeidx_symbol_slice` | symbol body range와 snippet_ref 조회 | 작음 |
...
| `codeidx_top_files` | matching file 또는 directory hit count. `group_by="directory"`와 `directory_depth`로 구현체/호출부 분포를 요약 | enabled |
...
```json
{
  ...
  "properties": {
    ...
    "output_mode": { "type": "string", "enum": ["minimal", "rg_like", "snippet", "structured"], "default": "minimal" },
    "include_diagnostics": { "type": "boolean", "default": false },
    "preview_max_chars": { "type": "integer", "minimum": 20, "maximum": 2000, "default": 96 },
    "limit": { "type": "integer", "minimum": 1, "maximum": 200, "default": 20 },
    "cursor": { "type": ["string", "null"], "default": null },
    "max_chars": { "type": "integer", "minimum": 1000, "maximum": 200000, "default": 24000 },
    "diagnostic_level": { "type": "string", "enum": ["compact", "full"], "default": "compact" },
...
3. 기본 출력은 `output_mode="minimal"`이며 `path:line`만 반환한다. broad search에서는 이 기본값이 `rg -n` preview보다 작아야 한다.
4. compact text 호출은 기본적으로 `structuredContent` diagnostics를 생략한다. engine/fallback/scope/timing metadata가 필요하면 `include_diagnostics=true`를 명시한다.
5. `output_mode="rg_like"`는 명시 요청 시에만 line preview를 포함하며, preview는 `preview_max_chars`로 줄인다.
6. `diagnostic_level="compact"`가 기본이다. compact diagnostics는 engine/fallback/scope semantics/ranking mode/phase timings만 유지하고, full diagnostics는 query terms와 verbose ranking metadata까지 포함한다.
7. timeout 응답은 `query_diagnostics.timeout_phase`와 `query_diagnostics.timings`를 포함해 backend search, overlay, ranking, snippet hydration 중 어디서 시간을 썼는지 드러낸다.
8. no-trigram regex는 자동 실행하되 경고와 강한 limit을 적용한다.
9. `require_fresh=true`면 dirty overlay 반영이 끝날 때까지 가능한 범위에서 refresh하거나 live scan을 병합한다.
10. stale 결과가 있으면 각 result와 summary에 표시한다.
11. `include_sensitive=false`가 기본값이다.
12. generated/dependency file은 기본 제외한다.
13. 결과 ranking에는 Zoekt score + symbol/edge proximity + file role + framework relevance를 함께 사용한다.
...
```json
{
  ...
  "properties": {
    ...
    "scope_preset": {
      "type": "string",
      "enum": ["source", "tests", "production", "all"],
      "default": "source"
    },
    "group_by": {
      "type": "string",
...
```json
{
  ...
  "scope": {
    ...
    "semantics": {
      ...
      "excludes_generated": true,
      "excludes_dependencies": true,
      "excludes_local_editor_context": true
    }
  },
  "counts": {
    "total": 49,
...
- `source`: production code와 tests를 포함하되 migrations/generated/dependencies/local editor context는 제외한다.
- `production`: `source`에서 tests도 제외한다.
- `tests`: tests만 포함하고 migrations/generated/dependencies/local editor context는 제외한다.
- `all`: preset 필터를 끄고 reference index가 가진 전체 범위를 반환한다.
...
## 8.12 Removed: `codeidx_refresh_index`

`codeidx_refresh_index`는 agent-facing MCP surface에서 제거한다. 대형 repo에서 refresh/rebuild는 오래 걸릴 수 있고, agent 탐색 요청이 index 작업을 시작하면 benchmark와 실제 작업 모두에 노이즈가 생긴다.

Index 준비는 사용자가 extension command나 CLI로 수행한다. MCP는 `codeidx_index_status`와 `mcp_health`로 freshness, `search_index_ready`, `last_engine_error`, storage 상태를 읽기 전용으로 보고한다.
...
Index refresh/rebuild는 파일을 직접 수정하지 않더라도 agent-facing MCP read-only 범주에 넣지 않는다. 큰 workspace에서 장시간 index 작업을 시작할 수 있으므로 extension command나 CLI에서만 수행한다.
...
4. Wait for extension-managed incremental indexing or prepare the index outside MCP.
...
- external index refresh readiness reporting
...
기능:
  ...
  - workspace_overview/index_status/search_code/search_symbols/resolve_at/symbol_details/symbol_slice/find_references/find_implementations/graph_neighbors/get_context_bundle/read_snippets가 동작한다.
...
2. `mcp_test` baseline 0 오판: 2026-05-06 수정 후, 2026-05-16 제거. 내부 bounded baseline이 실제 `rg`/agent 탐색 기준과 달라 벤치마크 노이즈가 크므로 MCP 도구 표면에서 제외한다.
